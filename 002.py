...
def input_dat(name):
    ...
    while x < 1 or x > 28:
        x = int(input(f"{name}, можно взять только до 28 конфет: "))
    return x


def p_print(name, k, counter, value):
    print(f"{name}, взял {k} конфет, теперь у него {counter} конфет. Осталось на столе {value} конфет.")
